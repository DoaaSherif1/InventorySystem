...
namespace EFFINAL
{
    public partial class AllImportsRequests : Form
    {
        EF Ent=new EF();
        public AllImportsRequests()
        {
            InitializeComponent();
        }

        private void AllImportsRequests_Load(object sender, EventArgs e)
        {
            foreach(var item in Ent.Requests)
            {
                if (item.Req_Type == "Import")
                {
